...
class TestMajorsAPI:
    ...
    def test_create_major(self, client):
        """새 전공 생성 테스트"""
        response = client.post("/majors/", json={"name": "신규전공", "code": "NEW"})
    ...
    def test_create_major_without_code(self, client):
        """코드 없이 전공 생성 테스트"""
        response = client.post("/majors/", json={"name": "코드없는전공"})
    ...
    def test_create_duplicate_major(self, client, sample_major):
        ...
        response = client.post(
            "/majors/", json={"name": sample_major.name, "code": "DUP"}
        )
    ...
    def test_update_major(self, client, sample_major):
        """전공 정보 업데이트 테스트"""
        response = client.put(
            f"/majors/{sample_major.id}", json={"name": "변경된전공명", "code": "CHG"}
        )
    ...
    def test_update_major_deactivate(self, client, sample_major):
        """전공 비활성화 테스트"""
        response = client.put(f"/majors/{sample_major.id}", json={"is_active": False})
    ...
    def test_update_nonexistent_major(self, client):
        ...
        response = client.put("/majors/99999", json={"name": "업데이트실패"})
