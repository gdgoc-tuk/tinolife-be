...
from app.domains.users.schema import UserCreate, SignupRequest
...
class UserService:
    ...
    def signup(self, db: Session, signup_data: SignupRequest) -> User:
        """
        회원가입 처리
    ...
    def add_user_interests(
        self, db: Session, user_id: int, interest_ids: List[int]
    ) -> None:
        """
        사용자 관심사 추가
    ...
    def update_user_interests(
        ...
        # 1. 기존 관심사 모두 제거
        db.execute(user_interests.delete().where(user_interests.c.user_id == user_id))
