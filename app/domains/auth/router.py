...
@router.post(
    "/allowed-domains",
    response_model=AllowedEmailDomainResponse,
...
async def create_allowed_domain(
    ...
    domain = (
        domain_data.domain
        if domain_data.domain.startswith("@")
        else "@" + domain_data.domain
    )
    existing = await service.get_domain_by_name(db, domain)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Domain already exists"
        )
...
async def send_verification_code(
    ...
    db: Session = Depends(get_db),
    domain_service: AllowedEmailDomainService = Depends(
        get_allowed_email_domain_service
    ),
    ...
    try:
        ...
        if verified:
            return VerifyCodeResponse(
                success=True, message="이메일 인증이 완료되었습니다.", verified=True
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="인증에 실패했습니다."
            )
    ...
    try:
        ...
        if not verification_service.is_email_verified(db, request.email):
            raise BadRequestException(
                "이메일 인증이 완료되지 않았습니다. 먼저 이메일 인증을 진행해주세요."
            )
